...
class ImageClassificationSystem:
    ...
    def __init__(self):
        ...
        self.main.title("Image Clasisifaction Using CNN")
    ...
    def generateModel(self):
        ...
        if os.path.exists('model.json'):
            ...
            messagebox.showinfo("Model Generated",
                                "CNN Model Generated on Train & Test Data. See black console for details")
        else:
            ...
            classifier.fit_generator(training_set,
                                     ...
                                     nb_epoch=5,  # i have changed nb_epoch 1 to 5
    ...
    def start_program(self):
        font = ('times', 16, 'bold')
        title = Label(self.main, text='Image Classification Using Deep Learning CNN Algorithm', justify=LEFT)
        ...
        font1 = ('times', 14, 'bold')
        model = Button(self.main, text="Generate CNN Train & Test Model", command=self.generateModel)
        model.place(x=200, y=100)
        model.config(font=font1)

        uploadimage = Button(self.main, text="Upload Test Image", command=self.upload)
        uploadimage.place(x=200, y=150)
        uploadimage.config(font=font1)

        classifyimage = Button(self.main, text="Classify Picture In Image", command=self.classify)
        classifyimage.place(x=200, y=200)
        classifyimage.config(font=font1)

        exitapp = Button(self.main, text="Exit", command=self.exit)
        exitapp.place(x=200, y=250)
        exitapp.config(font=font1)

        self.main.config(bg='light coral')
        self.main.mainloop()


user = ImageClassificationSystem()
user.start_program()
